    ...
    for line in lines:
        ...
        if len(data) >= 37:
         ...
         {COLOR_CODE["YELLOW"]}ТЕЛЕФОН: {phone}{COLOR_CODE["RED"]}
              |---Информация: {num + getc}
              |
              |---Теги: {b + l + a + g + f + fa + m + v + x + i + r + z + s + p + h + t + d + u + n + k + y + ta + bh + lh + ah + gh
                         + fah + uh + nh + kh + yh + tah
                }
...
def bilane(bilane_file, search_value):
    found = False
...
def sport(sport_file, sport1_file, sport2_file, search_value):
    found = False

    with open(sport_file, 'r', encoding='utf-8') as file:
        lines = file.readlines()
    ...
    if not found:
        ...
        with open(sport1_file, 'r', encoding='utf-8') as file:
            lines = file.readlines()
        ...
        if not found:
            ...
            with open(sport2_file, 'r', encoding='utf-8') as file:
                lines = file.readlines()
